...
class Nursewebservice extends CI_Controller{
  ...
  public function login(){
    ...
    if ($check_login) {
      ...
      $this->webservice_model->update_data('users',['lat' => $lat,'lon' => $lon, 'register_id' => $register_id],$login_where);
...
function search_user_lists()
{
  ...
  $search_user_lists = $this->db->query("SELECT * from users LIKE firstname '$keyword' DESC")->result_array();  
...
{ 
  ...
  /*if($status=='Current'){
   $request_care_lists = $this->db->query("SELECT * from request_care WHERE nurse_id='$nurse_id' AND status='Current' order by created_date DESC")->result_array();
 ...
 else
 {
	...
	$request_care_lists = $this->webservice_model->get_request($arr_whr);   
	//$request_care_lists = $this->webservice_model->get_where('request_care',$arr_whr)->order_by('created_date','DESC');
 ...
 if (!empty($request_care_lists)) {
  ...
  foreach($request_care_lists as $request_care_list)
  {
   ...
   $request_care_list['contact_address'] = $user_details[0]['address'];
   ...
   if($user_details[0]['image']!=''){
    ...
    $request_care_list['user_image'] = SITE_URL.'uploads/images/user.jpg';
  }
...
/*************  nurse_category_list *************/
public function nurse_category_list()
{ 

  $service_type_id = $this->input->get_post('service_type_id');                  

  $nurse_category_lists = $this->webservice_model->get_where('nurse_category',['service_type_id'=>$service_type_id]);


  if ($nurse_category_lists) {

    foreach($nurse_category_lists as $nurse_category_list)
    {

      $data[] = $nurse_category_list;
...
public function add_pricing_plan(){
 ...
 $pricing_details_id = $this->webservice_model->insert_data('pricing_details',$arr_pricing_data);
...
{ 
  ...
  $nurse_id = $this->input->get_post('nurse_id');                                       
	$get_user = $this->webservice_model->get_where('users',['id'=>$nurse_id]);
	if($get_user && isset($get_user[0])){
		$request_care_lists = $this->webservice_model->getAllAvailableRequest($get_user[0]['lat'],$get_user[0]['lon'],$nurse_id, 20);
		if(!empty($request_care_lists)){
			$ressult['result']=$request_care_lists;
		   	$ressult['message']='successful';
		   	$ressult['status']='1';
		   	$json = $ressult;
		}
...
public function request_details(){
  ...
  if (!empty($get_request_care))
  {
    ...
    $get_visit = $this->webservice_model->get_where('visits',['id'=>$get_request_care[0]['visit_id']]);
   ...
   $get_user_details = $this->webservice_model->get_where('users',['id'=>$get_request_care[0]['user_id']]);

   $distance = $this->webservice_model->distance($lat, $lon, $get_user_details[0]['lat'], $get_user_details[0]['lon'], $miles = false);

   $time =  ($distance * 2);
   $arrivel_time = $this->convertToHoursMins($time);

   if($get_user_details[0]['image']!=''){
    $get_user_details[0]['image'] = SITE_URL.'uploads/images/'.$get_user_details[0]['image'];
   }
   else{
    $get_user_details[0]['image'] = '';
   }
   
   $get_request_care[0]['user_details'] = $get_user_details[0];
   ...
   $get_request_care[0]['visit_type'] = $get_visit[0]['type'];
   $get_request_care[0]['distance'] = number_format($distance,2);
   $get_request_care[0]['arrivel_time'] = $arrivel_time;
   $ressult['result']=$get_request_care[0];
   $ressult['message']='successfull';
   $ressult['status']='1';
   $json = $ressult;
...
function request_accept(){
	...
	$arr_whr = ['id'=>$this->input->get_post('request_id'), 'nurse_id'=> "0"];

	$get_request_care = $this->webservice_model->get_where('request_care',$arr_whr);
	if ($get_request_care[0]['id'] == "")
	{
		$ressult['result']='Either data is not available or someone else has applied for the job.';
		$ressult['message']='unsuccessfull';
		$ressult['status']='0';
		$json = $ressult;
	...
	$update_request_care = $this->webservice_model->update_data('request_care',['status'=>$status,'updated_date'=>date('Y-m-d h:i:s'), 'nurse_id'=> $nurse_id],$arr_whr);

	$get_nurse = $this->webservice_model->get_where('users',['id'=>$nurse_id]);

    $get_user = $this->webservice_model->get_where('users',['id'=>$get_request_care[0]['user_id']]);

	$message = $get_nurse[0]['firstname'].' '.$get_nurse[0]['lastname']." will be available for your concern treatment";

	$user_message_apk = array(
		"message" => array(
			"result" => "successful",
			"key" => "Your request has been accepted",
			"request_id"=>$get_request_care[0]['id'],
			"mobile" => $get_nurse[0]['phone_number'],
			"body" => $message,
	...
	$register_userid = array(
		$get_user[0]['register_id']
	);

	$push_notification_response = $this->webservice_model->sendPushNotification($register_userid, $user_message_apk);
            
	$ressult['result']=$get_request_care[0];
	$result['push_response'] = $push_notification_response;
    $ressult['message']='successfull';
    $ressult['status']='1';
...
public function request_cancel(){
  ...
  $update_request_care = $this->webservice_model->update_data('request_care',['status'=>$status,'updated_date'=>date('Y-m-d h:i:s')],$arr_whr);
  if ($update_request_care)
  {
    $get_update_request_care = $this->webservice_model->get_where('request_care',$arr_whr);
    ...
    if($status=='Current'){
    	...
    	// Android Notification Code Start
      	$user_message_apk = array(
		    "message" => array(
		      "result" => "successful",
		      "key" => "Your request has been accepted",
		      "userid" => $get_nurse[0]['id'],
		      "username" => $get_nurse[0]['firstname'].' '.$get_user[0]['lastname'] ,
    	...
    	// Android Notification Code Start
      	$user_message_apk = array(
		    "message" => array(
		      "result" => "successful",
		      "key" => "Your request has been cancelled by the nurse.",
		      "userid" => $get_nurse[0]['id'],
		      "username" => $get_nurse[0]['firstname'].' '.$get_user[0]['lastname'] ,
		      "userimage" => SITE_URL . "uploads/images/" . $get_user[0]['image'],
		      "date" => date('Y-m-d h:i:s')
    ...
    $ressult['result']=$get_update_request_care[0];
    $ressult['message']='successfull';
    $ressult['status']='1';
    $json = $ressult;
  }
...
/************* update electronic notes for request *******************/
public function update_enote($update_status){
	$request_id = $this->input->get_post('request_id');
	$enote = $this->input->get_post('enote');
	$nurse_id = $this->input->get_post('nurse_id');

	$arr_whr = ['id'=>$request_id, 'nurse_id'=>$nurse_id];
	
  	$get_request_care = $this->webservice_model->get_where('request_care',$arr_whr);
	if(isset($get_request_care[0])){
		$updated_data = ['electronic_notes'=>$enote, 'updated_date'=>date('Y-m-d h:i:s')];
		if($update_status=="true"){
			$updated_data['status'] = 'Completed';	
		}

		$update_request_care = $this->webservice_model->update_data('request_care',$updated_data, $arr_whr);
		if($update_request_care){
			$get_request_care[0]['enote'] = $enote;
			$ressult['result']=$get_request_care[0];
			$ressult['message']='successfull';
			$ressult['status']='1';
		}
		else{
			$ressult['result']='Update failed. Please try again.';
			$ressult['message']='unsuccessfull';
			$ressult['status']='0';
		}
...
public function get_profile(){
  ...
  $get_nurse = $this->webservice_model->get_where('users',$arr_whr);

  if ($get_nurse) {  
    if($get_nurse[0]['image']!=''){
     $get_nurse[0]['image']=SITE_URL.'uploads/images/'.$get_nurse[0]['image'];
    }
    else{
     $get_nurse[0]['image']='';

    }
    $ressult['result']=$get_nurse[0];
    $ressult['message']='successfull';
    $ressult['status']='1';
    $json = $ressult;
...
public function user_update(){
  ...
  $arr_whr = ['id'=>$this->input->get_post('nurse_id')];
...
public function change_password()
{
  ...
  $id = $this->input->get_post('nurse_id', TRUE);

  $arr_login = ['id' => $id ,'password' => base64_encode($password)];
  $login = $this->webservice_model->get_where('users', $arr_login);
...
public function get_my_notification($userId){
	$get_user = $this->webservice_model->get_where('users',['id'=>$userId]);
	if($get_user && isset($get_user[0])){
		$notifications = $this->webservice_model->getNurseNotifications($get_user[0]['lat'],$get_user[0]['lon'],20,$userId);
		if($notifications){
...
public function get_current_notes($nurse_id){
	$get_user = $this->webservice_model->get_where('users',['id'=>$nurse_id]);
	if($get_user && isset($get_user[0])){
		$request_data = $this->webservice_model->getCurrentRequestNotes($nurse_id);
		if($request_data){
